...
private fun getFullProjectVersion(mcVersion: String?, modVersion: String): String {
    ...
    val subprojectName  = getSubprojectHash(mcVersion)
    ...
    return when {
        isRelease -> "${base}-${commitCount}-${subprojectName}-release"
        isPR      -> "${base}-${commitCount}-${subprojectName}-pr"
        else      -> "${base}-${currentBranch}-${
            if (isCI && buildNumber != null) "${commitCount}-${subprojectName}-ci"
            else "${timestampMillis}-${subprojectName}-development"
        }"
    }
}
